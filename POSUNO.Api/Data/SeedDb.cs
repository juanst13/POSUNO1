...
namespace POSUNO.Api.Data
{
    public class SeedDb
    {
        ...
        private async Task CheckUserAsync()
        {
            ...
            {
                _context.Users.Add(new User { Email = "juan@yopmail.com", FirstName = "Juan", LastName = "Gil", Password = "123456" });
        ...
        private async Task CheckCustomersAsync()
        {
            if (!_context.Products.Any())
            {
                ...
                for (int i = 1; i <= 50; i++)
                {
                    _context.Customers.Add(new Customer { FirstName = $"Cliente {i}", LastName = $"Apellido {i}", PhoneNumber = "3115550011", Address = "Cr 50 Cl 50 - 50", Email = $"cliente{i}@yopmail.com", IsActive = true ,User = user });
